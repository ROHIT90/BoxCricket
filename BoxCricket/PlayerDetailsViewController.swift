...
class PlayerDetailsViewController: UIViewController,UITableViewDataSource, UITableViewDelegate {
    ...
    deinit {
        self.ref.child("Vicky").removeObserver(withHandle: _refHandle)
    ...
    func configureDatabase() {
        ...
        _refHandle = self.ref.child("Vicky").observe(.childAdded, with: { [weak self] (snapshot) -> Void in
            guard let strongSelf = self else { return }
            strongSelf.Vicky.append(snapshot)
            strongSelf.clientTable.insertRows(at: [IndexPath(row: strongSelf.Vicky.count-1, section: 0)], with: .automatic)
        })
    }

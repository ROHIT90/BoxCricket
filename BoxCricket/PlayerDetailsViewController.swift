...
class PlayerDetailsViewController: UIViewController {
    ...
    deinit
    {
        self.ref.child("VickyMatches").removeObserver(withHandle: _refHandle)
        self.ref.child("VickyRuns").removeObserver(withHandle: _refHandle)
        self.ref.child("VickyWins").removeObserver(withHandle: _refHandle)
    }
    ...
    func configureMatchDatabase()
    {
        ...
        _refHandle = self.ref.child("VickyMatches").observe(.childAdded, with: { [weak self] (snapshot) -> Void in
    ...
    {
        ref = FIRDatabase.database().reference()
        _refHandle = self.ref.child("VickyRuns").observe(.childAdded, with: { [weak self] (snapshot) -> Void in
            guard let strongSelf = self else { return }
            strongSelf.vicky.append(snapshot)
    ...
    {
        ref = FIRDatabase.database().reference()
        _refHandle = self.ref.child("VickyWins").observe(.childAdded, with: { [weak self] (snapshot) -> Void in
            guard let strongSelf = self else { return }
            strongSelf.vicky.append(snapshot)
    ...
    func updateRuns(withData data: [String: String])
    {
        ...
        self.ref.child("VickyRuns").child("runs").setValue(data)
    }
    ...
    func updateMatch(withData data: [String: String])
    {
        //var mdata = data
        //mdata[Constants.MessageFields.name] = AppState.sharedInstance.displayName
        self.ref.child("VickyMatches").child("matches").setValue(data)
    }
    ...
    func updateWins(withData data: [String: String])
    {
        //var mdata = data
        //mdata[Constants.MessageFields.name] = AppState.sharedInstance.displayName
        self.ref.child("VickyWins").child("wins").setValue(data)
    }

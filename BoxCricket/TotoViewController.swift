...
class TotoViewController: UIViewController {
    ...
    deinit
    {
        self.ref.child("TotoMatches").removeObserver(withHandle: _refHandle)
        self.ref.child("TotoRuns").removeObserver(withHandle: _refHandle)
        self.ref.child("TotoWins").removeObserver(withHandle: _refHandle)
    }
    ...
    func configureMatchDatabase()
    {
        ...
        _refHandle = self.ref.child("TotoMatches").observe(.childAdded, with: { [weak self] (snapshot) -> Void in
            guard let strongSelf = self else { return }
            
            strongSelf.toto.append(snapshot)
    ...
    {
        ref = FIRDatabase.database().reference()
        _refHandle = self.ref.child("TotoRuns").observe(.childAdded, with: { [weak self] (snapshot) -> Void in
            guard let strongSelf = self else { return }
            strongSelf.toto.append(snapshot)
    ...
    {
        ref = FIRDatabase.database().reference()
        _refHandle = self.ref.child("TotoWins").observe(.childAdded, with: { [weak self] (snapshot) -> Void in
            guard let strongSelf = self else { return }
            strongSelf.toto.append(snapshot)
    ...
    func updateRuns(withData data: [String: String])
    {
        ...
        self.ref.child("TotoRuns").child("runs").setValue(data)
    }
    ...
    func updateMatch(withData data: [String: String])
    {
        //var mdata = data
        //mdata[Constants.MessageFields.name] = AppState.sharedInstance.displayName
        self.ref.child("TotoMatches").child("matches").setValue(data)
    }
    ...
    func updateWins(withData data: [String: String])
    {
        //var mdata = data
        //mdata[Constants.MessageFields.name] = AppState.sharedInstance.displayName
        self.ref.child("TotoWins").child("wins").setValue(data)
    }
